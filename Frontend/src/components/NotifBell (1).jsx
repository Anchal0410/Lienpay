...
//
// WHY createPortal?
// position:fixed breaks when any ancestor has transform/will-change/overflow.
// Dashboard has all three (parallax blobs, ScrollReveal, scrollRef).
// createPortal renders the backdrop + sheet directly into document.body,
// completely bypassing the parent stacking context.
//
...
const DEFAULT_PREFS = {
  ...
  nav_drops:           false,
  credit_activity:     true,
}

const MOCK_NOTIFS = [
  {
    id: 'n1', type: 'ltv_warnings', icon: '⚡', color: '#E0A030',
    title: 'LTV at 82% — Approaching Limit',
    body: 'Portfolio collateral dropped. Add funds or repay to avoid a margin call.',
    ts: Date.now() - 2 * 60 * 60 * 1000, read: false,
  },
  {
    id: 'n2', type: 'repayment_reminders', icon: '💳', color: '#00D4A1',
    title: 'Interest Due in 3 Days',
    body: '₹1,240 interest payment due on Apr 1. Auto-debit is set up.',
    ts: Date.now() - 5 * 60 * 60 * 1000, read: false,
  },
  {
    id: 'n3', type: 'credit_activity', icon: '↗', color: '#4DA8FF',
    title: 'Payment of ₹8,500 Processed',
    body: 'UPI payment to Swiggy via LienPay credit line.',
    ts: Date.now() - 22 * 60 * 60 * 1000, read: true,
  },
]
...
export default function NotifBell({ ltvStatus }) {
  ...
  // ── Portal content (backdrop + bottom sheet) ──────────────
  // Rendered into document.body to escape ALL parent stacking contexts
  const portalContent = (
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            ...
            <div style={{ padding: '8px 22px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 400, color: '#E8F0EC' }}>
                  Notifications
                </h2>
                {unread > 0 && (
                  <p style={{ fontSize: 11, color: '#3A4A44', marginTop: 2, fontFamily: 'var(--font-mono)' }}>
                    {unread} UNREAD
                  </p>
                )}
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                {tab === 'notifications' && unread > 0 && (
                  <button onClick={markAllRead}
                    style={{ fontSize: 11, color: '#00D4A1', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
                    Mark all read
                  </button>
                )}
            ...
            {/* Tab switcher */}
            <div style={{ display: 'flex', padding: '0 22px', marginBottom: 4, gap: 4 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '4px 22px 20px' }}>
              ...
              {tab === 'settings' && (
                <div>
                    ...
                    <div style={{ background: 'rgba(0,212,161,0.08)', border: '1px solid rgba(0,212,161,0.16)', borderRadius: 14, padding: '12px 14px', marginBottom: 14, display: 'flex', gap: 10 }}>
                      ...
                      <div>
                        ...
                        <button onClick={() => Notification?.requestPermission()}
                          style={{ marginTop: 10, padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: '#00D4A1', color: '#050809', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
                          Allow Notifications
                        </button>
                      </div>
                    </div>
                  ...
                  {PREF_CONFIG.map((p) => (
                    <div key={p.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 0', borderBottom: '1px solid rgba(0,212,161,0.06)' }}>
                      <div style={{ display: 'flex', gap: 11, alignItems: 'center', flex: 1 }}>
                        <div style={{
                          width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, transition: 'all 0.2s',
                          background: prefs[p.key] ? 'rgba(0,212,161,0.08)' : '#12181D',
                          border: prefs[p.key] ? '1px solid rgba(0,212,161,0.16)' : '1px solid rgba(0,212,161,0.06)',
                        }}>
                          {p.icon}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {p.critical && (
                              <span style={{ fontSize: 8, padding: '2px 6px', borderRadius: 4, background: 'rgba(224,82,82,0.10)', color: '#E05252', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>
                                CRITICAL
                              </span>
                            )}
                          </div>
                      ...
                      <button onClick={() => togglePref(p.key)}
                        style={{
                          width: 46, height: 26, borderRadius: 13, flexShrink: 0, position: 'relative', cursor: 'pointer',
                          background: prefs[p.key] ? '#00D4A1' : '#12181D',
                          border: prefs[p.key] ? 'none' : '1px solid rgba(0,212,161,0.10)',
                          transition: 'background 0.25s',
                        }}>
                        <motion.div
                          animate={{ x: prefs[p.key] ? 22 : 2 }}
                  ...
                  <p style={{ fontSize: 10, color: '#3A4A44', marginTop: 16, lineHeight: 1.6, fontFamily: 'var(--font-mono)' }}>
                    CRITICAL alerts are recommended at all times. Disabling them may result in missed collateral breach notices.
                  </p>
                </div>
              )}
            </div>
  ...
  return (
    <>
      {/* Bell button — stays in the normal DOM flow inside Dashboard */}
      <motion.button
        whileTap={{ scale: 0.88 }}
      ...
      {/* Backdrop + sheet — portalled into document.body */}
