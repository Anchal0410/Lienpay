...
export default function Profile({ onSettings }) {
  ...
  const handleLogout = async () => {
    ...
    // ── FIX: force page reload for clean state — prevents blank screen ──
    setTimeout(() => { window.location.href = '/' }, 400)
  }

  // ── FIX: backend already returns "+91 XXXXX12345" — don't re-prefix ──
    ...
    <div className="screen">
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 400, marginBottom: 4 }}>{displayName}</h2>
          <p style={{ fontSize: 12, color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{displayMobile}</p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          {items.map((item, i) => (
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', padding: '15px 18px', borderBottom: i < items.length - 1 ? '1px solid var(--border)' : 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 13, fontWeight: 600, fontFamily: item.label === 'UPI ID' ? 'var(--font-mono)' : 'var(--font-sans)' }}>{item.value}</p>
              </div>
            </div>
          ))}
        </motion.div>
