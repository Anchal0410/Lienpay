...
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      g1.addColorStop(0, 'rgba(0,212,161,0.10)'); g1.addColorStop(0.4, 'rgba(0,212,161,0.04)'); g1.addColorStop(1, 'transparent')
      ctx.fillStyle = g1; ctx.fillRect(0, 0, W, H)

      const g2 = ctx.createRadialGradient(W * 0.85 + Math.sin(t * 0.6) * 25, H * 0.75 + Math.cos(t * 0.8) * 20, 0, W * 0.85, H * 0.75, W * 0.4)
      g2.addColorStop(0, 'rgba(0,168,120,0.08)'); g2.addColorStop(0.5, 'rgba(0,168,120,0.03)'); g2.addColorStop(1, 'transparent')
      ctx.fillStyle = g2; ctx.fillRect(0, 0, W, H)

      ctx.strokeStyle = 'rgba(0,212,161,0.035)'; ctx.lineWidth = 1
      gridLines.forEach(l => {
        ctx.beginPath()
      ...
      rings.forEach(ring => {
        ...
        ctx.beginPath(); ctx.arc(ring.x, ring.y, ring.r * pulse, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(0,212,161,${0.04 + Math.sin(ring.phase) * 0.02})`; ctx.lineWidth = 1; ctx.stroke()
        ctx.beginPath(); ctx.arc(ring.x, ring.y, ring.r * pulse * 0.65, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(0,212,161,${0.025})`; ctx.stroke()
      })
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(0,212,161,${p.alpha * (0.7 + 0.3 * Math.sin(p.pulse))})`; ctx.fill()
      })
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 100) {
            ...
            ctx.strokeStyle = `rgba(0,212,161,${0.08 * (1 - d / 100)})`; ctx.lineWidth = 0.5; ctx.stroke()
          }
        }
      }
      ...
      sg.addColorStop(0, 'transparent'); sg.addColorStop(0.5, 'rgba(0,212,161,0.04)'); sg.addColorStop(1, 'transparent')
...
const TICKERS = [
  { label: 'NIFTY 50', val: '+0.84%', up: true }, { label: 'SENSEX', val: '+0.76%', up: true },
  { label: 'LTV CAP', val: '40%', up: null }, { label: 'PLEDGE', val: 'SECURE', up: null },
  { label: 'APR', val: '12%', up: null }, { label: 'GOLD', val: '+0.22%', up: true },
...
export default function Splash({ onComplete }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#050809', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
      <ParticleCanvas />

      {/* Market ticker */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, overflow: 'hidden', height: 28, borderBottom: '1px solid rgba(0,212,161,0.08)', background: 'rgba(5,8,9,0.6)', display: 'flex', alignItems: 'center' }}>
        <motion.div
      ...
      >
        ...
        <motion.div
          ...
          style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -70%)', width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,212,161,0.18) 0%, transparent 70%)', filter: 'blur(8px)' }}
        />

        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.25, duration: 0.9 }}
        ...
        <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7 }}
          style={{ fontFamily: 'var(--font-display)', fontSize: 40, fontWeight: 300, letterSpacing: '-1px', marginBottom: 6 }}>
        ...
        {/* ── FIX: ALL 4 words highlighted in jade ── */}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        style={{ position: 'absolute', bottom: 52, width: 100, height: 2, background: 'rgba(0,212,161,0.1)', borderRadius: 2, overflow: 'hidden' }}>
        <motion.div
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.35 }} transition={{ delay: 1, duration: 0.8 }}
