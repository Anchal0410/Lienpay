...
// ─────────────────────────────────────────────────────────────
// PREMIUM CITYSCAPE ANIMATION
// Smooth, calm, staggered building reveals + gentle orbital
// ─────────────────────────────────────────────────────────────
function CityscapeProcessing({ merchant, amount }) {
  // Building data — sorted by position
  const buildings = [
  ...
  const svgH    = 270
  const centerX = 240
  const centerY = groundY - 200  // center of LienPay building mid-section

  return (
    <div style={{ position: 'absolute', inset: 0, background: '#040c08', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

      {/* Amount + Merchant — fade in gently */}
      <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: 'easeOut', delay: 0.2 }}
        style={{ textAlign: 'center', paddingTop: 28, paddingBottom: 6, position: 'relative', zIndex: 10 }}>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.9 }} transition={{ delay: 0.4, duration: 1 }}
          style={{ fontSize: 9, color: 'var(--jade)', fontFamily: 'var(--font-mono)', letterSpacing: '4px', fontWeight: 700, marginBottom: 8 }}>LIENPAY</motion.p>
        <motion.p initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5, duration: 0.9, ease: [0.16,1,0.3,1] }}
          style={{ fontFamily: 'var(--font-display)', fontSize: 46, fontWeight: 400, color: '#fff', letterSpacing: '-1px', marginBottom: 4 }}>{fmt(amount)}</motion.p>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} transition={{ delay: 0.8, duration: 0.8 }}
          style={{ fontSize: 13, color: '#8fa99a', fontFamily: 'var(--font-sans)' }}>to {merchant}</motion.p>
      </motion.div>
        ...
        <svg viewBox={`0 0 500 ${svgH}`} preserveAspectRatio="xMidYMax meet"
          ...
          <defs>
            <linearGradient id="skyGrad2" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#030a06"/>
              <stop offset="100%" stopColor="#050f09"/>
            </linearGradient>
            <linearGradient id="bldGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#0d1e16"/>
              <stop offset="100%" stopColor="#081209"/>
            </linearGradient>
            <linearGradient id="centerGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#0f2e1c"/>
              <stop offset="100%" stopColor="#071508"/>
            </linearGradient>
            <radialGradient id="centerGlow" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="rgba(0,212,161,0.12)"/>
              <stop offset="100%" stopColor="rgba(0,212,161,0)"/>
            </radialGradient>
            <radialGradient id="groundGlow" cx="50%" cy="0%" r="80%">
              <stop offset="0%" stopColor="rgba(0,212,161,0.05)"/>
              <stop offset="100%" stopColor="rgba(0,212,161,0)"/>
            </radialGradient>
          </defs>

          {/* Sky */}
          <rect width="500" height={svgH} fill="url(#skyGrad2)"/>

          {/* Stars — gentle twinkle */}
          {stars.map((s, i) => (
            <motion.circle key={i} cx={s.x} cy={s.y} r={s.r} fill="white"
              animate={{ opacity: [0.15, 0.55, 0.15] }}
              transition={{ duration: 3 + (i % 3), repeat: Infinity, delay: s.delay, ease: 'easeInOut' }} />
          ))}

          {/* Buildings — staggered reveal from ground up */}
          {buildings.map((b, i) => (
            <motion.g key={i}
              initial={{ opacity: 0, y: 40 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1.4, delay: 0.1 + i * 0.07, ease: [0.16, 1, 0.3, 1] }}>
              <rect x={b.x} y={groundY - b.h} width={b.w} height={b.h} fill="url(#bldGrad)"/>
              {b.windows.map((w, j) => (
                <motion.rect key={j} x={b.x + w[0]} y={groundY - b.h + w[1]} width={5} height={4}
                  fill={j % 4 === 0 ? 'rgba(255,185,60,0.55)' : j % 7 === 0 ? 'rgba(255,220,100,0.35)' : 'rgba(255,255,255,0.05)'}
                  rx={0.3}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ delay: 0.6 + i * 0.07 + j * 0.015, duration: 0.6 }} />
              ))}
            </motion.g>
          ))}

          {/* Central LienPay building — rises last, smoothly */}
          <motion.g initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.8, delay: 0.8, ease: [0.16, 1, 0.3, 1] }}>
            {/* Glow behind building */}
            <ellipse cx={centerX} cy={groundY - 100} rx={60} ry={80} fill="url(#centerGlow)"/>
            {/* Building */}
            <rect x={204} y={groundY - 230} width={72} height={230} fill="url(#centerGrad)" rx={0.5}/>
            {/* Building windows */}
            {[0,1,2,3,4,5,6].map(row => (
              [0,1,2,3].map(col => (
                <motion.rect key={`${row}-${col}`}
                  x={208 + col * 16} y={groundY - 220 + row * 28} width={9} height={7}
                  fill={row % 2 === 0 && col % 2 === 0 ? 'rgba(0,212,161,0.2)' : 'rgba(255,255,255,0.03)'}
                  rx={1}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ delay: 1.2 + row * 0.06 + col * 0.03, duration: 0.8 }} />
              ))
            ))}
            {/* "L" — fades in last */}
            <motion.text x={240} y={groundY - 55} textAnchor="middle" fill="#00D4A1" fontSize="26" fontFamily="Georgia, serif" fontWeight="300"
              initial={{ opacity: 0 }} animate={{ opacity: 0.9 }} transition={{ delay: 1.6, duration: 1.2 }}>L</motion.text>
            {/* Soft glow behind L */}
            <motion.circle cx={240} cy={groundY - 65} r={20} fill="rgba(0,212,161,0.07)"
              animate={{ r: [18, 24, 18] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 2 }} />
          </motion.g>

          {/* Orbital ring — draws in slowly after building rises */}
          <motion.circle cx={centerX} cy={groundY - 165} r={68} fill="none"
            stroke="rgba(0,212,161,0.14)" strokeWidth="0.7"
            initial={{ pathLength: 0, opacity: 0 }}
            animate={{ pathLength: 1, opacity: 1 }}
            transition={{ delay: 1.8, duration: 2.0, ease: 'easeInOut' }} />

          {/* Orbiting dot — smooth, slow, calm */}
          <motion.g
            animate={{ rotate: [0, 360] }}
            transition={{ duration: 8, repeat: Infinity, ease: 'linear', delay: 2.5 }}
            style={{ transformOrigin: `${centerX}px ${groundY - 165}px` }}>
            <motion.circle cx={centerX} cy={groundY - 165 - 68} r={5} fill="#00D4A1"
              animate={{ r: [4, 6, 4] }}
              transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}/>
            <circle cx={centerX} cy={groundY - 165 - 68} r={10} fill="rgba(0,212,161,0.12)"/>
          </motion.g>

          {/* Secondary smaller orbit */}
          <motion.circle cx={centerX} cy={groundY - 165} r={44} fill="none"
            stroke="rgba(0,212,161,0.06)" strokeWidth="0.5"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.4, duration: 1.5 }} />

          {/* Antenna dots — staggered blink */}
          {[
            { bldIdx: 0, color: '#EF4444' }, { bldIdx: 2, color: '#EF4444' },
            { bldIdx: 3, color: '#EF4444' }, { bldIdx: 6, color: '#EF4444' },
            { bldIdx: 7, color: '#00D4A1' }, { bldIdx: 8, color: '#EF4444' },
          ].map((d, i) => {
            const b = buildings[d.bldIdx]
            return (
              <motion.circle key={i} cx={b.x + b.w / 2} cy={groundY - b.h - 5} r={2.5} fill={d.color}
                initial={{ opacity: 0 }}
                animate={{ opacity: [0, 0.9, 0.3, 0.9, 0] }}
                transition={{ duration: 2.5, repeat: Infinity, delay: 1.0 + i * 0.4, ease: 'easeInOut' }} />
            )
          })}
          ...
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 1.5 }}>
            ...
            <line x1={0} y1={groundY} x2={500} y2={groundY} stroke="rgba(0,212,161,0.12)" strokeWidth="0.5"/>
            {/* Ground glow */}
            <rect x={130} y={groundY} width={240} height={30} fill="url(#groundGlow)"/>
          </motion.g>
        </svg>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 1 }}
        style={{ textAlign: 'center', paddingBottom: 16, position: 'relative', zIndex: 10 }}>
        <motion.p
          animate={{ opacity: [0.4, 0.8, 0.4] }}
          transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
          style={{ fontSize: 9, color: 'rgba(0,212,161,0.6)', fontFamily: 'var(--font-mono)', letterSpacing: '2px' }}>
          PROCESSING PAYMENT
        </motion.p>
      </motion.div>

      {/* Progress bar — smooth linear fill */}
      <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 2.8, ease: [0.4, 0, 0.2, 1] }}
        style={{ height: 2, background: 'linear-gradient(90deg, var(--jade), rgba(0,212,161,0.6))', position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 20, transformOrigin: 'left center' }} />
    </div>
  )
}
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        setSpendingData({ categories: sorted, total, count: txns.length })
      } catch(e) {}
    }
    init()
  }, [])
    ...
    <div style={{ position: 'fixed', inset: 0, paddingTop: TICKER_HEIGHT, background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: step === 'processing' || step === 'success' ? 'hidden' : 'auto', position: 'relative' }}>
        <AnimatePresence mode="wait">
          ...
          {/* PROCESSING — premium cityscape */}
          {step === 'processing' && txnData && (
            <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} exit={{ opacity: 0 }}
              style={{ position: 'absolute', inset: 0 }}>
              <CityscapeProcessing merchant={txnData.merchant_name} amount={txnData.amount || parseFloat(confirmAmount) || 0} />
            </motion.div>
          )}

          {/* SUCCESS — cityscape + overlay */}
          {step === 'success' && txnResult && (
            <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} exit={{ opacity: 0 }}
              style={{ position: 'absolute', inset: 0 }}>
              <CityscapeProcessing merchant={txnResult.merchant} amount={txnResult.amount} />
              <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.8, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                style={{ position: 'absolute', bottom: 36, left: 20, right: 20, background: 'rgba(4,12,8,0.92)', border: '1px solid rgba(0,212,161,0.2)', borderRadius: 24, padding: '24px 20px', textAlign: 'center', backdropFilter: 'blur(16px)' }}>
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1.0, type: 'spring', stiffness: 200 }}
                  ...
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17L4 12" stroke="var(--jade)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </motion.div>
                <p style={{ fontFamily: 'var(--font-display)', fontSize: 22, marginBottom: 4, color: '#fff' }}>Payment Sent</p>
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 20 }}>30-day interest-free period started</p>
                <motion.button whileTap={{ scale: 0.97 }} onClick={onSuccess} style={{ width: '100%', height: 50, borderRadius: 16, background: 'linear-gradient(135deg, var(--jade), #00A878)', color: '#000', fontSize: 14, fontWeight: 800, fontFamily: 'var(--font-sans)', border: 'none', cursor: 'pointer' }}>Done</motion.button>
              </motion.div>
            </motion.div>
          )}
