    ...
    <div className="screen">
      <div style={{padding:'16px 20px 0'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          ...
          <p style={{fontSize:10,color:'var(--jade)',fontFamily:'var(--font-mono)',fontWeight:600}}>{pledgedH.length} / {holdings.length} funds</p>
        </div>
        ...
        {pledgedH.length===0 ? (
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16}}>
            {pledgedH.map((h,i) => {
              const c=sC(h.scheme_type), el=parseFloat(h.eligible_value||h.eligible_credit||0)
              return (
                  ...
                  <div style={{marginLeft:8}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
                      <div style={{flex:1,minWidth:0,paddingRight:8}}>
                        <p style={{fontSize:13,fontWeight:600,marginBottom:4,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{h.scheme_name}</p>
                      ...
                      <p style={{fontSize:14,fontWeight:600,fontFamily:'var(--font-mono)',flexShrink:0}}>{fC(h.value_at_fetch)}</p>
                    </div>
                    <div style={{display:'flex',justifyContent:'space-between',borderTop:'1px solid var(--border)',paddingTop:10}}>
                      ...
                      <div><p style={{fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-mono)',marginBottom:3}}>NAV</p><p style={{fontSize:12,color:'var(--text-secondary)'}}>Rs.{parseFloat(h.nav_at_fetch||0).toFixed(2)}</p></div>
        ...
        {/* Add more collateral — always visible when unpledged funds exist */}
        {unpledgedH.length>0 && (
          <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:0.2}} style={{marginBottom:16}}>
            <button onClick={()=>setShowAdd(v=>!v)}
              style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderRadius:showAdd?'14px 14px 0 0':14,background:showAdd?'rgba(0,212,161,0.05)':'var(--bg-surface)',border:showAdd?'1px solid var(--jade-border)':'1px solid var(--border)',cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',gap:12}}>
                <div style={{width:34,height:34,borderRadius:10,background:'var(--jade-dim)',border:'1px solid var(--jade-border)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,color:'var(--jade)',lineHeight:1}}>+</div>
                <div style={{textAlign:'left'}}>
                  <p style={{fontSize:13,fontWeight:600,color:'var(--jade)'}}>Add more collateral</p>
                  <p style={{fontSize:11,color:'var(--text-muted)',marginTop:1}}>{unpledgedH.length} fund{unpledgedH.length!==1?'s':''} available to pledge</p>
                </div>
              </div>
              <span style={{fontSize:12,color:'var(--text-muted)',display:'inline-block',transition:'transform 0.2s',transform:showAdd?'rotate(180deg)':'none'}}>v</span>
            </button>
            <AnimatePresence>
              {showAdd && (
                <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.25}}
                  ...
                  <div style={{padding:'10px 12px',background:'var(--bg-surface)',borderBottom:'1px solid var(--border)'}}>
                    <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by fund name or RTA..."
        ...
        {pledgeModal && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:28,stiffness:340}}
              ...
              {(()=>{
                const h=pledgeModal, c=sC(h.scheme_type), el=parseFloat(h.eligible_value||h.eligible_credit||0)
                return (
