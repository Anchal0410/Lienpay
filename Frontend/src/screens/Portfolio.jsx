...
export default function Portfolio() {
  ...
  const isPledged = (folio) => { const p = pledgeMap[folio]; return p && p.status === 'ACTIVE' }
  ...
  return (
    <div className="screen">
      <div style={{ padding: '16px 20px 0' }}>
        ...
        <p style={{ fontSize:13, color:'var(--text-secondary)', marginBottom:20 }}>Your collateral health</p>

        {/* ── CREDIT SUMMARY CARD ── */}
        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
          style={{ background:'linear-gradient(135deg,rgba(0,212,161,0.07),rgba(0,212,161,0.02))', border:'1px solid var(--jade-border)', borderRadius:20, padding:'18px 20px', marginBottom:16 }}>

          {/* Top row: available + outstanding */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
            <div>
              ...
              <p style={{ fontFamily:'var(--font-display)', fontSize:24, fontWeight:400, color:'var(--jade)' }}>
                {creditLimit > 0 ? formatCurrency(available) : '—'}
              </p>
            </div>
            <div>
              <p style={{ fontSize:10, color:'var(--text-muted)', letterSpacing:'2px', fontFamily:'var(--font-mono)', marginBottom:4 }}>OUTSTANDING</p>
              <p style={{ fontFamily:'var(--font-display)', fontSize:24, fontWeight:400, color: outstanding > 0 ? 'var(--amber)' : 'var(--text-secondary)' }}>
                {creditLimit > 0 ? formatCurrency(outstanding) : '—'}
              </p>
            </div>
          </div>

          {/* Bottom row: credit limit + portfolio value */}
        ...
        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1 }}
          style={{ background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:16, padding:'14px 16px', marginBottom:16 }}>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
              ...
              <p style={{ fontSize:11, fontWeight:600, fontFamily:'var(--font-mono)',
            ...
            <div style={{ height:6, borderRadius:3, background:'var(--bg-elevated)', overflow:'hidden', position:'relative' }}>
              ...
              <motion.div initial={{ width:0 }} animate={{ width:`${Math.min(outstanding>0?ltvRatio:0,100)}%` }}
                transition={{ duration:1, ease:'easeOut' }}
                style={{ height:'100%', borderRadius:3,
                  background: ltvRatio>=90 ? 'linear-gradient(90deg,#EF4444,#DC2626)' : ltvRatio>=80 ? 'linear-gradient(90deg,var(--jade),#F59E0B)' : 'linear-gradient(90deg,var(--jade),#00A878)' }}/>
            </div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
          <p style={{ fontSize:12, color:'var(--text-muted)', letterSpacing:'0.5px' }}>MUTUAL FUND HOLDINGS</p>
            ...
            {holdings.map((holding, i) => {
              ...
              return (
                <motion.div key={holding.folio_number}
                  ...
                  style={{ background:'var(--bg-surface)', border:`1px solid ${pledged?'var(--jade-border)':'var(--border)'}`, borderRadius:14, padding:'14px 16px', position:'relative', overflow:'hidden', cursor:pledged?'default':'pointer' }}>
                  <div style={{ position:'absolute', left:0, top:0, bottom:0, width:3, background:pledged?'var(--jade)':color }}/>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                    <div style={{ flex:1, marginLeft:8 }}>
                      <p style={{ fontSize:13, fontWeight:500, color:'var(--text-primary)', marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:200 }}>
                        {holding.scheme_name}
                      </p>
                      <div style={{ display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
                        <span style={{ fontSize:9, color, fontWeight:600, letterSpacing:'0.5px', background:`${color}15`, padding:'2px 6px', borderRadius:4 }}>
                          {holding.scheme_type?.replace(/_/g,' ')}
                        </span>
                        <span style={{ fontSize:10, color:'var(--text-muted)' }}>{holding.rta}</span>
                        {pledged
                          ? <span style={{ fontSize:8, fontWeight:700, fontFamily:'var(--font-mono)', color:'var(--jade)', background:'var(--jade-dim)', border:'1px solid var(--jade-border)', padding:'2px 6px', borderRadius:4 }}>PLEDGED</span>
                          : <span style={{ fontSize:8, fontWeight:600, fontFamily:'var(--font-mono)', color:'var(--text-muted)', background:'var(--bg-elevated)', border:'1px solid var(--border-light)', padding:'2px 6px', borderRadius:4 }}>TAP TO PLEDGE</span>
                        }
                      </div>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      <p style={{ fontSize:15, fontWeight:600 }}>{formatCurrency(holding.value_at_fetch)}</p>
                      <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:2 }}>{pct.toFixed(1)}% of portfolio</p>
                    </div>
                  </div>
                  <div style={{ display:'flex', justifyContent:'space-between', marginLeft:8, paddingTop:8, borderTop:'1px solid var(--border)' }}>
                    <div>
                      <p style={{ fontSize:10, color:'var(--text-muted)', marginBottom:2 }}>UNITS</p>
                      <p style={{ fontSize:12, color:'var(--text-secondary)' }}>{parseFloat(holding.units_held||0).toFixed(3)}</p>
                    </div>
                    <div>
                      <p style={{ fontSize:10, color:'var(--text-muted)', marginBottom:2 }}>NAV</p>
                      <p style={{ fontSize:12, color:'var(--text-secondary)' }}>₹{parseFloat(holding.nav_at_fetch||0).toFixed(2)}</p>
                    </div>
                    <div>
                      <p style={{ fontSize:10, color:'var(--text-muted)', marginBottom:2 }}>LTV</p>
                      <p style={{ fontSize:12, color }}>{formatLtv(holding.ltv_cap)}</p>
                    </div>
                    <div>
                      <p style={{ fontSize:10, color:'var(--text-muted)', marginBottom:2 }}>ELIGIBLE</p>
                      <p style={{ fontSize:12, color:'var(--jade)' }}>{formatCurrency(holding.eligible_value || holding.eligible_credit)}</p>
                    </div>
                  </div>
                </motion.div>
              )
      ...
      <AnimatePresence>
        {pledgeModal && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            style={{ position:'fixed', inset:0, zIndex:50, background:'rgba(5,8,9,0.85)', display:'flex', alignItems:'flex-end' }}
            ...
            <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:'spring',damping:28,stiffness:340}}
              ...
              <p style={{ fontSize:12, color:'var(--text-secondary)', lineHeight:1.6, marginBottom:16 }}>
                Your units will be lien-marked (not sold). Investments keep growing. Release anytime by repaying outstanding balance.
              </p>
              <div style={{ display:'flex', gap:10 }}>
                <motion.button whileTap={{scale:0.96}} onClick={()=>setPledgeModal(null)} disabled={pledging}
                  style={{ flex:1, height:50, borderRadius:14, background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)', fontSize:14, fontWeight:600, cursor:'pointer' }}>
                  Cancel
                </motion.button>
                <motion.button whileTap={{scale:0.96}} onClick={()=>handlePledge(pledgeModal)} disabled={pledging}
                  style={{ flex:1, height:50, borderRadius:14, background:'linear-gradient(135deg,var(--jade),#00A878)', color:'var(--bg-void)', fontSize:14, fontWeight:700, border:'none', cursor:'pointer', boxShadow:'0 6px 20px rgba(0,212,161,0.2)', opacity:pledging?0.6:1 }}>
