...
export default function Dashboard({ onPay }) {
  ...
  // Compute bell state for the small button
  ...
  return (
    <>
      ...
      <div ref={scrollRef} className="screen">
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '0 20px' }}>
          ...
          <div style={{ paddingTop: 18, paddingBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', transform: `translateY(${headerY}px)` }}>
            ...
            {/* Bell — only opens bottom sheet, no floating panel */}
      ...
      {/* Notification bottom sheet — renders outside scroll container, no overlap ever */}
      <NotificationSheet
        creditAccount={account}
        ltvHealth={ltv}
        open={notifOpen}
        ...
        {showSimulator && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 280, damping: 32 }}
              ...
              <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 16, color: 'var(--jade)' }}>Risk Simulator</p>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                Simulate how changes in portfolio value or utilisation affect your LTV ratio.
                LTV = Outstanding ÷ Max Eligible Credit × 100.
              </p>
              <div style={{ marginTop: 16, background: 'var(--bg-surface)', borderRadius: 14, padding: '14px 16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[
                    { label: 'Credit Limit',    value: fmtL(creditLimit) },
                    { label: 'Outstanding',      value: fmtL(outstanding) },
                    { label: 'Current LTV',      value: `${ltvRatio.toFixed(1)}%` },
                    { label: 'Available Credit', value: fmtL(available) },
                  ].map((s, i) => (
                    <div key={i} style={{ background: 'var(--bg-elevated)', borderRadius: 10, padding: '10px 12px' }}>
                      <p style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: 4 }}>{s.label.toUpperCase()}</p>
                      <p style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--jade)' }}>{s.value}</p>
                    </div>
                  ))}
                </div>
              </div>
